...
module Spec
  module Helpers
    ...
    def sanitize_test_dir
      return unless @test_dir
      FileUtils.rm_rf(File.join(@test_dir, 'Gemfile.lock'))
    ...
    def write_gemfile(additional=nil)
      contents = <<~EOF.chomp
      ...
      File.write(test_dir.join("Gemfile"), contents)
      lockfile_path = test_dir.join("Gemfile.lock")
      File.delete(lockfile_path) if File.exist?(lockfile_path)
    end

    def lockfile
      file = test_dir.join("Gemfile.lock")
    ...
    def lockfile_dep_names_for_spec(spec_name)
      return unless (lf = lockfile)
      spec = lf.specs.detect { |s| s.name == spec_name}
      spec.dependencies.map { |d| d.name }
    end
    ...
    def raw_bundle(command, verbose: false, env: {})
      ...
      out, err, process_status = Helpers.with_unbundled_env do
        Open3.capture3(env, command, :chdir => test_dir)
      end
    ...
    def bundle(command, expect_error: false, verbose: true, env: {})
      ...
      if verbose
        puts "\n#{'=' * 80}\n#{command}\n#{'=' * 80}\n#{bundler_output}\n#{'=' * 80}\n"
        puts "Gemfile.lock:\n#{File.read(test_dir.join("Gemfile.lock"))}" if File.exist?(test_dir.join("Gemfile.lock"))
